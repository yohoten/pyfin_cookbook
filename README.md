...
- **在线主页**：[GitHub Pages 介绍网站](https://yohoten.github.io/pyfin_cookbook/)（仓库根目录 `index.html`；`manual.html` 为手册在线阅读器，版式参照《Hello 算法》文档站，支持目录定位、全文检索、插图放大与上下章跳转）
...
├── reports/                        # 课程实践报告（模板与成稿，含个人信息，已排除版本控制）
│
├── index.html                      # GitHub Pages 介绍主页（含手册目录与附录速查）
├── manual.html                     # 手册在线阅读器（由第三稿 DOCX 程序化生成）
├── assets/                         # 站点样式 / 脚本 / 阅读器插图
...
**在线阅读器**：仓库根目录的 `manual.html` 由本地构建脚本从第三稿 DOCX 程序化生成（脚本不在仓库内），内容与第三稿同源——前言 + 8 章正文 + 附录 A–K、144 节、114 张插图、117 张表格；正文中标注「◻ 演示图占位」的位置为有意留白的截图位，并附补图提示。版式参照《Hello 算法》文档站：左栏全书目录（可筛选标题）、右栏本页目录、滚动定位、上下章翻页（`Alt` + `←`/`→`）、插图点击放大；顶部「检索正文」（快捷键 `/` 或 `Ctrl/⌘ + K`）对**全书正文**做全文检索，命中结果带所在章节、高亮片段与定位跳转，检索索引由前端从正文 DOM 即时建立，不增加页面体积。
...
- **`reports/`**：课程实践报告模板与本人在读期间的成稿（DOCX / PDF）。因含姓名、学号等个人信息，已整体排除在版本控制之外，也不在介绍网站上提供下载；网站上对应位置改为指向手册第 7 章的写作指导。
...
- `reports/`（课程实践报告，含个人信息）已整体写入忽略规则并移出版本控制，不在仓库与网站上分发。
